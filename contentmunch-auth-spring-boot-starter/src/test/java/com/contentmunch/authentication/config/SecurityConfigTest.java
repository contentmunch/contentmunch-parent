...
@SpringBootTest(
        properties = {
            ...
            "contentmunch.auth.users.testuser.name=Contentmunch",
            "contentmunch.auth.users.testuser.id=1",
            ...
            "contentmunch.auth.users.testuser.enabled=true",
...
class SecurityConfigTest {
    ...
    void meShouldReturnUserWithValidToken() throws Exception {
        String token = tokenizationService.generateAccessToken(ContentmunchUser.builder()
                .id(1L)
                .enabled(true)
                .username("testuser")
                .password("password")
